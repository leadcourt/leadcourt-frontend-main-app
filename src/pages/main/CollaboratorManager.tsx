...
import { getAllInvitations, getAllSentInvitations, inviteUser } from '../../utils/api/collaborationAPI';
  ...
  const onSubmit = async (values: AddCollaboratorData) => {
    ...
    await inviteUser(payload).then((res)=>{
      ...
      if (res?.status == 201){
        ...
        getInvitations();
        getCollaborators();
      } else if (res?.status == 200 && res?.data?.message === 'Invite already sent') {
        toast.info('Invitation already sent to this email.');


      } else if (res?.status == 200 && res?.data?.message === 'User is already a collaborator') {
        toast.info('User is already a collaborator.');
      
      }else if (res?.status == 400) {
        toast.error('Invitation failed! User not found. Please check the email address.');
       } else {
        toast.error('Failed to send invitation. Confirm email and Please try again.');
      }
