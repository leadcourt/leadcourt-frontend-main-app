...
import { connectionHubspotCRM } from "../../utils/api/crmIntegrations";

import {
  collaboration_checkHubspotConnection,
  collaboration_checkBrevoConnection,
  collaboration_connectBrevo,
  collaboration_removeBrevoConnection,
...
const Collab_Integrations = () => {
    ...
    () => [
      {
        ...
        enabled: true,
        connection:
          connections?.hubspot?.connected === true
            ? true
            : connections?.hubspot === "checking"
            ? "checking"
            : false,
      ...
      },
      {
        id: "brevo",
        name: "Brevo",
        description: "Export your lists to Brevo contact lists",
        icon: (
        ...
        enabled: true,
        connection:
          connections?.brevo?.connected === true
            ? true
            : connections?.brevo === "checking"
            ? "checking"
            : false,
        connectionLink: null,
      },
  ...
  const hubspotConnection = async () => {
    ...
    await collaboration_checkHubspotConnection()
      .then((response) => {
  ...
  const brevoConnection = async () => {
    ...
    await collaboration_checkBrevoConnection(false)
      .then((response) => {
  ...
  const handleBrevoConnect = async () => {
    ...
    setBrevoSaving(true);
    await collaboration_connectBrevo({ apiKey: key })
      .then((res) => {
  ...
  const handleBrevoDisconnect = async () => {
    setBrevoSaving(true);
    await collaboration_removeBrevoConnection()
      .then((res) => {
        if (res?.data?.success) {
      ...
      >
        <div className="w-full">
          {connections?.brevo?.connected ? (
            ...
            <div className="flex flex-col gap-4 p-2">
              <div className="text-sm text-gray-700">Paste your Brevo API key to connect.</div>
