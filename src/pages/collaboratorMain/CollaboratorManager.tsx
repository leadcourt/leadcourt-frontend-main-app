...
import {
  Users, UserPlus, Mail, Check, AlertCircle, Settings, Crown, Eye, Edit3, Trash2, Search, X
} from "lucide-react";
...
interface AddCollaboratorData { email: string; role: string; }
interface Collaborator {
  id: string; name: string; email: string;
  role: "owner" | "admin" | "editor" | "viewer";
  status: "accepted" | "pending" | "declined";
  avatar?: string; joinedDate: string; lastActive: string;
}
...
const CollaboratorManager: React.FC = () => {
  ...
  const [loadingDeleteCollab, setLoadingDeleteCollab] = useState<{loading: boolean; collab: string}>();
  ...
  const getAvatarStyle = (index: number) => {
    const styles = [
      "bg-purple-100 text-purple-600", "bg-emerald-100 text-emerald-600",
      "bg-blue-100 text-blue-600", "bg-amber-100 text-amber-600", "bg-rose-100 text-rose-600"
    ];
  ...
  // API Logic
  const getInvitations = async () => { await getAllInvitations().then(() => {}); };
    ...
    if (action === "remove" && selectedCollaborators.length > 0) {
      ...
      try {
        await Promise.all(idsToDelete.map(async (id) => {
          ...
          if (res.status === 204) setCollaborators((prev) => prev.filter((c) => c.id !== id));
        }));
        setSelectedCollaborators([]);
        toast.success("Selected collaborators removed");
      } catch (err) { toast.error("Some collaborators could not be removed."); }
    }
  };

  const toggleSelectCollaborator = (id: string) => {
    setSelectedCollaborators((prev) => prev.includes(id) ? prev.filter((prevId) => prevId !== id) : [...prev, id]);
  };
  ...
  const onSubmit = async (values: AddCollaboratorData) => {
    ...
    } catch (err) { toast.error("Error Occurred, try again!"); }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] px-6 sm:px-10 py-8 font-sans">
      ...
      <Dialog visible={showInviteModal} onHide={() => setShowInviteModal(false)} showHeader={false} style={{ width: "420px" }} contentStyle={{ padding: "0", borderRadius: "16px", overflow: "hidden" }}>
        <div className="p-6 bg-white">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl font-bold text-gray-900">Add Collaborator</h2>
            <button onClick={() => setShowInviteModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors"><X size={20} /></button>
          </div>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[14px] font-medium text-gray-700">Email Address</label>
              <input name="email" type="email" value={values.email} onChange={handleChange} onBlur={handleBlur} placeholder="Enter email address" className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-[15px] outline-none focus:border-[#F35114] focus:ring-1 focus:ring-[#F35114] transition-all" />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="role" value={values.role} onChange={handleChange} onBlur={handleBlur} className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-[15px] bg-white outline-none focus:border-[#F35114]">
            ...
            <div className="flex justify-end gap-3 mt-4">
              ...
              <button type="submit" disabled={!isValid || isSubmitting} className="px-5 py-2.5 rounded-lg text-[14px] text-white bg-[#F35114] hover:bg-[#d84812] font-medium flex items-center gap-2 transition-colors shadow-sm disabled:opacity-50">
                {loading ? <i className="pi pi-spinner pi-spin"></i> : <Mail size={16} />} Send Invite
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[15px] text-gray-500 mt-0.5">Manage team members and their permissions</p>
            </div>
          </div>
          <div>
            {creditInfo?.subscriptionType.toLowerCase() === "pro" || creditInfo?.subscriptionType.toLowerCase() === "business" ? (
              ...
              <button onClick={() => navigate("/subscription")} className="bg-orange-600 hover:bg-orange-700 text-white text-[14px] px-6 py-2.5 rounded-full font-medium transition-colors">
                Upgrade Account
              </button>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
            <div className="relative w-full sm:w-[320px]">
              ...
              <input type="text" placeholder="Search collaborators..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full rounded-full border border-gray-200 bg-white py-2 px-4 pl-10 text-[14px] outline-none focus:border-[#F35114] transition-all shadow-sm" />
            </div>
            {selectedCollaborators?.length > 0 && (
              <div className="flex gap-2">
                <button onClick={() => handleBulkAction("remove")} className="bg-red-50 hover:bg-red-100 text-red-600 font-semibold px-4 py-2 rounded-xl text-[13px] transition-colors flex items-center gap-1.5 border border-red-100">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px]">
              <thead className="bg-white border-b border-gray-100">
                <tr>
                  <th className="w-[5%] py-4 px-6 text-left">
                    <input type="checkbox" checked={selectedCollaborators?.length === collaborators?.length && collaborators?.length > 0} onChange={(e) => { if (e.target.checked) { setSelectedCollaborators(collaborators?.map((c) => c.id)); } else { setSelectedCollaborators([]); } }} className="rounded border-gray-300 text-[#F35114] focus:ring-[#F35114]" />
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  [1, 2, 3].map((k) => <tr key={k}><td colSpan={5} className="py-6 px-6"><Skeleton height="2rem" /></td></tr>)
                ) : filteredCollaborators?.length ? (
                  filteredCollaborators?.map((collaborator, idx) => (
                    <tr key={collaborator.id} className="hover:bg-gray-50/60 transition-colors">
                      <td className="py-4 px-6">
                        {collaborator.role !== "owner" && (
                          <input type="checkbox" checked={selectedCollaborators.includes(collaborator.id)} onChange={() => toggleSelectCollaborator(collaborator.id)} className="rounded border-gray-300 text-[#F35114] focus:ring-[#F35114]" />
                        )}
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-[14px] shrink-0 ${getAvatarStyle(idx)}`}>
                            {collaborator.avatar}
                          </div>
                          <div>
                            <div className="text-[15px] font-bold text-gray-900">{collaborator.name}</div>
                            <div className="text-[13px] text-gray-500 font-medium">{collaborator.email}</div>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-bold border ${roleColors[collaborator.role] || roleColors.viewer}`}>
                          <RoleIcon role={collaborator.role} />
                          {collaborator.role.toUpperCase()}
                        </span>
                      </td>
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-[12px] font-bold ${statusColors[collaborator.status] || "bg-gray-100 text-gray-600"}`}>
                      ...
                      <td className="py-4 px-6 text-right">
                        {collaborator.role !== "owner" && (
                          <button onClick={() => handleRemoveCollaborator(collaborator.id)} className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Remove collaborator">
                            {loadingDeleteCollab?.collab === collaborator.id ? <i className="pi pi-spinner pi-spin"></i> : <Trash2 size={18} />}
                          </button>
                        )}
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={5} className="py-20 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-400">
                        ...
                        <p className="text-sm font-medium">No collaborators found.</p>
                      </div>
                    </td>
                  </tr>
                )}
